...
function Detail() {
  ...
  const setCommentChildren = (list)=>{
    console.log(list)
    const parentComment = list.filter(
      (item) => item.id_comment === 0
    );
    // console.log(parentComment);
    const listComment = parentComment.map((item) => {
      const children= list.filter(
        (comment) => comment.id_comment === item.id
      );
      const currentChildren = item.children || []
      return { ...item, children:[...currentChildren,...children] };
    });
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        setCommentChildren(res.data.data?.comment);
      });
  }, []);
  ...
  return (
    <div className="col-sm-9">
      ...
      {/*Comments*/}
      <BlogComment comments={comment} setComment={setCommentChildren}  />
    </div>
  );
}
